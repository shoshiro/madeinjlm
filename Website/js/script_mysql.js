...
function initialize() {
    ...
    for (var i = 0; i < data.companies.length; i++)
    {
        ...
        imgObj.OuterHTML = company.picture;
        ...
        if(markers[i].position && company.Industry)
        {
            company.Industry = company.Industry.toLowerCase();
            categories[company.Industry] += 1;
        }


        infoWindows[i] = new google.maps.InfoWindow({
            content: '<div class="map_infowindow">'+
                (company.picture ? '<div class="thumb">'+company.picture.replace("<img ","<img class='img-rounded' ")+'</div>' : '') +
                '<div class="info_content">'+
                '<b><a target="_blank" href="'+company.url+'">'+company.CompanyName+'</a></b>'+
